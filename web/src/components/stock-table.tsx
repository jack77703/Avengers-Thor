...
interface SortIconProps {
    columnKey: keyof Stock;
    activeKey?: keyof Stock;
    direction?: 'asc' | 'desc';
}

function SortIcon({ columnKey, activeKey, direction }: SortIconProps) {
    if (activeKey !== columnKey) return <Minus className="w-3 h-3 text-gray-300 opacity-0 group-hover:opacity-50" />;
...
export function StockTable({ stocks }: StockTableProps) {
    ...
    const sortedStocks = useMemo(() => {
        ...
        const getComparable = (stock: Partial<Stock>): number | string | null => {
            if (key === 'sentiment') return stock.sentiment?.bullish ?? null;
                ...
                <thead className="text-xs text-gray-500 uppercase bg-gray-50 dark:bg-gray-800 dark:text-gray-400">
                    <tr>
                        <th className="px-4 py-3 cursor-pointer group w-20" onClick={() => requestSort('rank')}>
                            <div className="flex items-center gap-1">Rank <SortIcon columnKey="rank" activeKey={sortConfig?.key} direction={sortConfig?.direction} /></div>
                        </th>
                        <th className="px-4 py-3 cursor-pointer group w-24" onClick={() => requestSort('trendingScore')}>
                            <div className="flex items-center gap-1">Trend <SortIcon columnKey="trendingScore" activeKey={sortConfig?.key} direction={sortConfig?.direction} /></div>
                        </th>
                        ...
                        <th className="px-4 py-3 text-right cursor-pointer group w-32" onClick={() => requestSort('price')}>
                            <div className="flex items-center justify-end gap-1">Price <SortIcon columnKey="price" activeKey={sortConfig?.key} direction={sortConfig?.direction} /></div>
                        </th>
                        <th className="px-4 py-3 text-right cursor-pointer group w-28" onClick={() => requestSort('percentChange')}>
                            <div className="flex items-center justify-end gap-1">% Chg <SortIcon columnKey="percentChange" activeKey={sortConfig?.key} direction={sortConfig?.direction} /></div>
                        </th>
                        <th className="px-4 py-3 text-right cursor-pointer group" onClick={() => requestSort('messageVolume')}>
                            <div className="flex items-center justify-end gap-1">Msg Vol (24h) <SortIcon columnKey="messageVolume" activeKey={sortConfig?.key} direction={sortConfig?.direction} /></div>
                        </th>
                        <th className="px-4 py-3 text-center cursor-pointer group w-24" onClick={() => requestSort('zScore')}>
                            <div className="flex items-center justify-center gap-1">Z-Score <SortIcon columnKey="zScore" activeKey={sortConfig?.key} direction={sortConfig?.direction} /></div>
                        </th>
                        <th className="px-4 py-3 text-center cursor-pointer group" onClick={() => requestSort('sentiment')}>
                            <div className="flex items-center justify-center gap-1">Bull Ratio <SortIcon columnKey="sentiment" activeKey={sortConfig?.key} direction={sortConfig?.direction} /></div>
                        </th>
                        ...
                        <th className="px-4 py-3 text-right cursor-pointer group w-24" onClick={() => requestSort('intradayVolatility')}>
                            <div className="flex items-center justify-end gap-1">Volat. <SortIcon columnKey="intradayVolatility" activeKey={sortConfig?.key} direction={sortConfig?.direction} /></div>
                        </th>
